...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
	...
	$result = mysqli_query($connection, "SELECT * FROM clc WHERE email='$email'");
...
//					  VALUES ('$name', '$email', '$password', '$address1', '$address2' , '$city' , '$state', '$postcode')"); // Insert query
		$stmt = $connection->prepare("INSERT INTO clc(name, email, password, address1, phone, city, state, postcode, specialisation) 
			VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param("sssssssss", $name,$email,$password,$address1,$phone,$city,$state,$postcode, $sList);
		if($stmt->execute()){
//if($query){
